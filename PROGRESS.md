...
### 반복 5 — 요청: 게임 창 키우기 (AC-15)
- 캔버스가 브라우저 창 전체를 채우고, 창 크기에 맞춰 0.25 단위로 확대(가로 26칸·세로 16칸 이상 보이게), 고해상도 화면 대응.
- E2E 테스트 추가: 창 전체 채움, 1600×1000으로 키우면 확대율 증가. 기존 중앙 정렬 검사는 고정값(480,320) 대신 실제 화면 크기 기준으로 변경.

### 반복 6 — 요청: 웹에서 바로 실행
- 클라우드 컨테이너의 서버는 외부에서 접속할 수 없어, 같은 World 를 브라우저 안에서 돌리는 단일 파일판(client/local-server.ts, client/offline.ts)을 추가하고 웹 페이지로 게시.
- E2E 추가: 단일 파일판 접속·이동·채팅·새로고침 후 위치 유지.

### 반복 7 — 요청: 더 밝고 귀여운 만화풍 그래픽
- 원작 리소스·디자인은 쓰지 않고 스타일만 참고: 2등신 치비 캐릭터(큰 눈·외곽선), 둥근 몬스터와 통통 튀는 애니메이션, 건물·나무를 개체로 그리기, 길·물가 둥근 가장자리, 테두리 데미지 숫자·타격 불꽃·레벨업 기둥, 미니맵·하단 상태바·단축 슬롯, 밝은 로그인 화면.
- 스크린샷 검토로 발견해 수정: 돌길 타일 이음새 선, 화면을 가리던 채팅창(최근 줄만 표시·흐려짐).
- 테스트 추가: 건물 묶기·가장자리 마스크(AC-15). verify 87/87, 심판 25/25.
